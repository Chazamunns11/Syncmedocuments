# Tend — Technical Specification

Master spec for the Tend coach-first CRM. Companion docs:
...
- Coach connects **their own Stripe** (Stripe Connect Standard, or their API keys). Money flows
  coach→client directly; **Tend takes no payment surcharge** (key differentiator vs Kajabi).
...
- Separately, Stripe Billing handles **Tend's own subscription** (Solo/Pro/Studio) + metered overage.
